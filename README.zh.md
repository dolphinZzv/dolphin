...
dolphin 提供内置的斜杠命令，在所有传输层中均可使用。在会话中输入 `/help` 可查看完整列表。
...
| `/skills [sub]` | 列出技能。子命令：`new`、`delete`、`show` |
...
子命令的使用方式如 `/skills new`。使用 `/help <command>` 查看具体命令的详细用法。

...
func TestGenerateSummary(t *testing.T) {
	...
	err = sess.GenerateSummary(dir, 12, 2, 0, "completed", "", 0, 0, 0)
	if err != nil {
...
func TestGenerateSummaryZeroTurns(t *testing.T) {
	...
	err = sess.GenerateSummary(dir, 0, 0, 0, "user_exit", "", 0, 0, 0)
	if err != nil {
		t.Fatalf("GenerateSummary: %v", err)
	}
...
func TestGenerateSummaryStates(t *testing.T) {
	...
	for _, state := range states {
		...
		defer sess.Close()
		err = sess.GenerateSummary(dir, 0, 0, 0, state, "", 0, 0, 0)
		if err != nil {
...
func TestGenerateSummaryFileContent(t *testing.T) {
	...
	err = sess.GenerateSummary(dir, 7, 1, 0, "completed", "", 0, 0, 0)
	if err != nil {
		t.Fatalf("GenerateSummary: %v", err)
	}
...
func TestGenerateSummaryBadDir(t *testing.T) {
	...
	err := sess.GenerateSummary("/nonexistent/path/that/does/not/exist", 0, 0, 0, "completed", "", 0, 0, 0)
	if err == nil {
